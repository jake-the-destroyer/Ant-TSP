...
'''
def updatePherTable(destination):

  for node in range(len(pheramone_table)):
...
def move(ant, destination):
  ...
  for neighbor in pheramone_table[current_node][destination]:
    next_nodes.append(neighbor['probability'])

  neighbor = next_nodes.index(max(next_nodes))
  next_city = pheramone_table[current_node][destination][neighbor]['neighbor']
  if(next_city in ant):
    ant_dead = True
  else:
    ant_dead = False
...
def updateTrailsGlobal(ant, degree_satisfied):
  ...
  for vertex in two_d_plane:
    for i in vertex:
      if i != None:
        i['pheramone'] = (1 - evaporation_rate) * i['pheramone']

  for vertex in range(len(ant) -1):
    from_point = min(ant[vertex], ant[vertex + 1])
    to_point =   max(ant[vertex], ant[vertex + 1])

    two_d_plane[from_point][to_point]['pheramone'] += included_cost
    two_d_plane[to_point][from_point]['pheramone'] += included_cost
...
def buildTree(tree, source, destination):
  ...
  while current_node != destination:
    ...
    for i in pheramone_table[current_node][destination]:
      ...
      next_neighbors.append(i['neighbor'])

    i = 0
    while i < len(next_neighbors):
      if next_neighbors[i] in visited:
        next_neighbors.pop(i) 
        next_nodes.pop(i)
      else:
        i += 1
    ...
    if len(next_neighbors) != 0:
      backtrack = 1
      neighbor = next_nodes.index(max(next_nodes))
      current_node = next_neighbors[neighbor]
      visited.append(current_node)
      if (prev_node, current_node) not in tree and (current_node, prev_node) not in tree:
        tree.append((prev_node,current_node))

    else:
      if current_node not in visited:
        visited.append(current_node)
      current_node = visited[-backtrack]
      backtrack += 1
...
'''
Method used to find the total tour length
'''
def findCost(tree):
  cost = 0
  for i in tree:
    cost += two_d_plane[i[0]][i[1]]['length']
...
count_map, initial_two_d_plane, necessary_points, node_degree, x1_coords, x2_coords, y1_coords, y2_coords  \
                     = graphToMatrix(hannan_graph, count_map)

pheramone_table = createPheramoneTable(initial_two_d_plane, node_degree)
'''
...
while len(necessary_points) > 0:
  two_d_plane = initial_two_d_plane
